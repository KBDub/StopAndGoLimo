...
<div class="bg-charcoal overflow-hidden flex flex-col">
    ...
    <div class="p-5 flex flex-col gap-3" style="background-color: var(--color-charcoal)">
        <span class="inline-block text-sm font-bold uppercase tracking-widest" style="color: var(--color-sunburst)">{{ $tag }}</span>
        <h3 class="text-white font-bold text-lg leading-snug">{{ $title }}</h3>
        <p class="text-base leading-relaxed" style="color: rgba(255,255,255,0.75)">{{ $desc }}</p>
        <a
            href="{{ $href }}"
            class="mt-1 inline-flex items-center gap-2 px-5 py-3 text-base font-bold text-charcoal transition-opacity hover:opacity-90 self-start"
