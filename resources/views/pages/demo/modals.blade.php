...
<main class="max-w-5xl mx-auto px-6 py-16 space-y-20">
    ...
    <div class="text-center">
        <h1 class="text-4xl font-bold text-charcoal mb-3">Modal System</h1>
        <div class="h-1 w-24 bg-sunburst mx-auto mb-4"></div>
        <p class="text-charcoal-light text-lg max-w-2xl mx-auto">
            <code class="text-sm bg-linen-dark px-1.5 py-0.5 rounded">x-ui.modal</code>,
            <code class="text-sm bg-linen-dark px-1.5 py-0.5 rounded">x-ui.modal-trigger</code>, and
            <code class="text-sm bg-linen-dark px-1.5 py-0.5 rounded">x-ui.contact-modal</code>
            — all variants live below. The gold FAB is anchored bottom-right.
        </p>
    </div>
    ...
    <section>
        ...
        <div class="flex flex-wrap gap-3">
            <x-ui.modal-trigger modal="demo-default"
                class="px-5 py-2.5 bg-linen-dark text-charcoal text-sm font-semibold rounded hover:bg-linen transition-colors">
                Default
            </x-ui.modal-trigger>

            <x-ui.modal-trigger modal="demo-dark"
                class="px-5 py-2.5 bg-charcoal text-white text-sm font-semibold rounded hover:bg-charcoal-dark transition-colors">
                Dark
            </x-ui.modal-trigger>

            <x-ui.modal-trigger modal="demo-gold"
                class="px-5 py-2.5 bg-gold-gradient text-charcoal text-sm font-semibold rounded hover:shadow-gold transition-all">
                Gold
            </x-ui.modal-trigger>

            <x-ui.modal-trigger modal="demo-success"
                class="px-5 py-2.5 bg-success text-white text-sm font-semibold rounded hover:opacity-90 transition-opacity">
                Success
            </x-ui.modal-trigger>

            <x-ui.modal-trigger modal="demo-warning"
                class="px-5 py-2.5 bg-warning text-charcoal text-sm font-semibold rounded hover:opacity-90 transition-opacity">
                Warning
            </x-ui.modal-trigger>

            <x-ui.modal-trigger modal="demo-danger"
                class="px-5 py-2.5 bg-error text-white text-sm font-semibold rounded hover:opacity-90 transition-opacity">
                Danger
            </x-ui.modal-trigger>
        </div>
    </section>
    ...
    <x-ui.modal name="demo-default" title="Default Variant" variant="default" size="md">
        ...
        <x-slot:footer>
            <x-ui.modal-trigger modal="demo-default" as="close"
                class="px-4 py-2 text-sm font-semibold text-charcoal-light border border-linen-dark rounded hover:bg-linen transition-colors">
                Cancel
            </x-ui.modal-trigger>
            <button class="px-5 py-2 bg-gold-gradient text-charcoal text-sm font-semibold rounded hover:shadow-gold transition-all">
                Confirm
            </button>
        </x-slot:footer>
    </x-ui.modal>
    ...
    <x-ui.modal name="demo-dark" title="Dark Variant" variant="dark" size="md">
        ...
        <x-slot:footer>
            <x-ui.modal-trigger modal="demo-dark" as="close"
                class="px-4 py-2 text-sm font-semibold text-[#aaa] border border-white/20 rounded hover:bg-white/10 transition-colors">
                Cancel
            </x-ui.modal-trigger>
            <button class="px-5 py-2 bg-gold-gradient text-charcoal text-sm font-semibold rounded hover:shadow-gold transition-all">
                Confirm
    ...
    <x-ui.modal name="demo-success" title="Success Variant" variant="success" size="md">
        ...
        <x-slot:footer>
            <x-ui.modal-trigger modal="demo-success" as="close"
                class="px-5 py-2 bg-success text-white text-sm font-semibold rounded hover:opacity-90 transition-opacity">
                Done
            </x-ui.modal-trigger>
        </x-slot:footer>
    </x-ui.modal>
    ...
    <x-ui.modal name="demo-warning" title="Warning Variant" variant="warning" size="md">
        ...
        <x-slot:footer>
            ...
            <button class="px-5 py-2 bg-warning text-charcoal text-sm font-semibold rounded hover:opacity-90 transition-opacity">
                Keep Working
            </button>
        </x-slot:footer>
    </x-ui.modal>
    ...
    <x-ui.modal name="demo-danger" title="Danger Variant" variant="danger" size="md">
        ...
        <x-slot:footer>
            ...
            <button class="px-5 py-2 bg-error text-white text-sm font-semibold rounded hover:opacity-90 transition-opacity">
                Delete Permanently
            </button>
        </x-slot:footer>
    </x-ui.modal>
        ...
        <div class="flex flex-wrap gap-3">
            @foreach(['sm' => 'SM — 22rem', 'md' => 'MD — 34rem (default)', 'lg' => 'LG — 46rem', 'xl' => 'XL — 58rem', 'full' => 'Full — 95vw'] as $sz => $label)
                <x-ui.modal-trigger modal="demo-size-{{ $sz }}"
                    class="px-5 py-2.5 bg-white border border-linen-dark text-charcoal text-sm font-semibold rounded hover:bg-linen transition-colors shadow-sm">
                    {{ $label }}
                </x-ui.modal-trigger>
            @endforeach
        </div>
    ...
    <x-ui.modal name="demo-dismiss-no" title="Non-dismissible Modal" :dismissible="false">
        ...
        <x-slot:footer>
            <x-ui.modal-trigger modal="demo-dismiss-no" as="close"
                class="px-5 py-2 bg-gold-gradient text-charcoal text-sm font-semibold rounded hover:shadow-gold transition-all">
                I Acknowledge
            </x-ui.modal-trigger>
        </x-slot:footer>
    </x-ui.modal>
    ...
    <section>
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <x-ui.modal-trigger modal="demo-header-override"
                class="px-5 py-2.5 bg-azure text-white text-sm font-semibold rounded hover:bg-azure-dark transition-colors">
        ...
        <x-slot:header>
            <div class="flex items-center gap-3 px-5 py-4 bg-azure border-b-2 border-azure-dark">
                ...
                <x-ui.modal-trigger modal="demo-header-override" as="close"
                    class="flex items-center justify-center w-8 h-8 rounded-full text-white/70 hover:bg-white/10 transition-colors">
    ...
    <x-ui.modal name="demo-rich-footer" title="Rich Footer Example" variant="gold">
        ...
        <x-slot:footer>
            ...
            <button class="px-5 py-2 bg-charcoal text-white text-sm font-semibold rounded hover:bg-charcoal-dark transition-colors">
                Save Draft
            </button>
            <button class="px-5 py-2 bg-gold-gradient text-charcoal text-sm font-semibold rounded hover:shadow-gold transition-all">
                Submit Order
    ...
    {{-- ═══════════════════════════════════════════════════════════════════ --}}
    {{-- SECTION 6: Contact modal FAB note --}}
    {{-- ═══════════════════════════════════════════════════════════════════ --}}
    <section class="bg-white rounded-lg p-8 shadow-gold border-l-4 border-sunburst">
        <h2 class="text-2xl font-bold text-charcoal mb-3">Anchored Contact FAB</h2>
        <p class="text-charcoal-light mb-2">
            <code class="text-sm bg-linen-dark px-1.5 py-0.5 rounded">x-ui.contact-modal</code> renders a fixed
            floating action button anchored to the <strong>bottom-right</strong> of the viewport with a pulsing
            gold halo. It is always visible on this page — scroll down to see it.
        </p>
        <p class="text-sm text-charcoal-lighter">
            Usage: <code class="bg-linen-dark px-1.5 py-0.5 rounded">&lt;x-ui.contact-modal /&gt;</code> — drop it anywhere in the layout.
        </p>
    </section>
