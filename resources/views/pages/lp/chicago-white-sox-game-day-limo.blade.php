<x-layouts.lp
    title="Elite Chicago White Sox Limo Service & Rate Field Party Buses"
    metaDescription="Tailgate-ready party buses and stretch limos to Rate Field. Stop & Go Airport Shuttle Service, Inc. picks up your South Side group from anywhere in Chicagoland. Flat rates locked at booking. Reserve your ride today."
    ogImage="/images/special-events/white-sox/stopngolimo-chicago-white-sox-rate-field-sunset.jpg"
    ogImageAlt="Rate Field at sunset, served by Stop & Go Airport Shuttle Service, Inc. for Chicago White Sox game day transportation"
>

    {{-- ─── Hero ─────────────────────────────────────────────────────────────── --}}
    <section id="lp-hero"
             style="position: relative; min-height: 82vh; display: flex; align-items: center; justify-content: center;
                    background-image: url('/images/special-events/white-sox/stopngolimo-chicago-white-sox-rate-field-sunset.jpg');
                    background-size: cover; background-position: center center; scroll-margin-top: 80px;">
        <div style="position: absolute; inset: 0; background: rgba(21, 21, 44, 0.78);"></div>
        <div style="position: relative; z-index: 1; max-width: 860px; margin: 0 auto; padding: 4rem 1.5rem; text-align: center;">
            <h1 class="font-head"
                style="font-size: clamp(2.2rem, 6vw, 3.75rem); font-weight: 400; color: var(--white); line-height: 1.15; margin-bottom: 1.25rem;">
                Elite Chicago White Sox Limo Service
                <strong style="display: block; color: var(--champagne); font-weight: 700;">
                    &amp; Rate Field Party Buses
                </strong>
            </h1>
            <p class="font-body"
               style="font-size: clamp(1rem, 2.5vw, 1.2rem); color: var(--cloud); line-height: 1.7; margin-bottom: 2.25rem; max-width: 700px; margin-left: auto; margin-right: auto;">
                We deliver the ultimate South Side game day experience. Luxury party buses, stretch limos, and executive sprinter vans with transparent flat rates for all of Chicagoland.
            </p>
            <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap;">
                <button onclick="window.dispatchEvent(new Event('open-modal-quote'))"
                        class="font-head"
                        style="background: var(--champagne); color: var(--navy); border: none;
                               padding: 1rem 2.25rem; font-size: 1.05rem; font-weight: 700; cursor: pointer; line-height: 1.4;">
                    Reserve Tailgate Bus
                </button>
                <a href="tel:+17083154445"
                   class="font-head"
                   style="background: transparent; color: var(--champagne); border: 2px solid var(--champagne);
                          padding: 1rem 2.25rem; font-size: 1.05rem; font-weight: 700; text-decoration: none; display: inline-block; line-height: 1.4;">
                    Call (708) 315-4445 Now
                </a>
            </div>
        </div>
    </section>

    {{-- ─── Trust Badges ──────────────────────────────────────────────────────── --}}
    <section id="lp-trust"
             style="background: var(--champagne); padding: 1.15rem 1.5rem; scroll-margin-top: 80px;">
        <div style="max-width: 1100px; margin: 0 auto;
                    display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));
                    gap: 0.75rem 1.5rem; text-align: center;">
            <p class="font-head"
               style="color: var(--navy); font-weight: 700; font-size: 0.9rem; margin: 0; line-height: 1.5;">
                Hometown New Lenox Company
            </p>
            <p class="font-head"
               style="color: var(--navy); font-weight: 700; font-size: 0.9rem; margin: 0; line-height: 1.5;">
                24/7 Dispatch Available
            </p>
            <p class="font-head"
               style="color: var(--navy); font-weight: 700; font-size: 0.9rem; margin: 0; line-height: 1.5;">
                Transparent Flat Rates
            </p>
            <p class="font-head"
               style="color: var(--navy); font-weight: 700; font-size: 0.9rem; margin: 0; line-height: 1.5;">
                Fully Insured Luxury Fleet
            </p>
        </div>
    </section>

    {{-- ─── Value Proposition Grid ────────────────────────────────────────────── --}}
    <section id="lp-value-props"
             style="background: var(--white); padding: 4.5rem 1.5rem; scroll-margin-top: 80px;">
        <div style="max-width: 1100px; margin: 0 auto;">
            <div style="width: fit-content; margin: 0 auto 1.5rem;">
                <h2 class="font-head"
                    style="font-size: clamp(1.75rem, 4vw, 2.75rem); font-weight: 400; color: var(--navy); text-align: center; line-height: 1.25;">
                    Why South Siders Choose
                    <strong style="font-weight: 700; color: var(--champagne);">Stop &amp; Go</strong>
                </h2>
                <div style="height: 3px; background: var(--champagne); width: 116%; max-width: calc(100vw - 3rem); margin-top: 0.85rem;"></div>
            </div>
            <p class="font-body"
               style="color: var(--slate); font-size: 1rem; line-height: 1.75; text-align: center; max-width: 640px; margin: 0 auto 2.75rem;">
                From tailgate setup to the final out, your group rides in comfort and arrives back home without a hitch. Here is what every White Sox ride includes.
            </p>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.5rem;">
                <div style="border-left: 4px solid var(--champagne); background: var(--navy); padding: 2rem 1.5rem;">
                    <h3 class="font-head"
                        style="color: var(--champagne); font-size: 1.15rem; font-weight: 700; margin-bottom: 0.75rem; line-height: 1.35;">
                        Tailgate Ready
                    </h3>
                    <p class="font-body"
                       style="color: var(--cloud); font-size: 0.95rem; line-height: 1.7; margin: 0;">
                        Spacious storage bays hold all your coolers, chairs, and pre-game gear. Load up in your driveway and ride to Rate Field without leaving anything behind. No cramped trunks. No gear left at home.
                    </p>
                </div>
                <div style="border-left: 4px solid var(--champagne); background: var(--navy); padding: 2rem 1.5rem;">
                    <h3 class="font-head"
                        style="color: var(--champagne); font-size: 1.15rem; font-weight: 700; margin-bottom: 0.75rem; line-height: 1.35;">
                        Direct Highway Access
                    </h3>
                    <p class="font-body"
                       style="color: var(--cloud); font-size: 0.95rem; line-height: 1.7; margin: 0;">
                        Smooth express transit via I-80, I-55, and I-90/94. Our chauffeurs know the Dan Ryan in and out, including the best post-game exit routes so your group avoids the parking lot traffic on 35th Street.
                    </p>
                </div>
                <div style="border-left: 4px solid var(--champagne); background: var(--navy); padding: 2rem 1.5rem;">
                    <h3 class="font-head"
                        style="color: var(--champagne); font-size: 1.15rem; font-weight: 700; margin-bottom: 0.75rem; line-height: 1.35;">
                        Luxury &amp; Safety
                    </h3>
                    <p class="font-body"
                       style="color: var(--cloud); font-size: 0.95rem; line-height: 1.7; margin: 0;">
                        Professional, background-checked chauffeurs handle all the driving so your group stays safe and focused on the game. Wrap-around leather seating, climate control, and Bluetooth audio make every mile part of the experience.
                    </p>
                </div>
            </div>
        </div>
    </section>

    {{-- ─── Mid-Page CTA ───────────────────────────────────────────────────────── --}}
    <section id="lp-cta-mid"
             style="background: var(--navy); padding: 4rem 1.5rem; text-align: center; scroll-margin-top: 80px;">
        <div style="max-width: 720px; margin: 0 auto;">
            <div style="width: fit-content; margin: 0 auto 1.5rem;">
                <h2 class="font-head"
                    style="font-size: clamp(1.6rem, 4vw, 2.5rem); font-weight: 400; color: var(--white); line-height: 1.25;">
                    Ready to Book Your
                    <strong style="font-weight: 700; color: var(--champagne);">South Side Ride?</strong>
                </h2>
                <div style="height: 3px; background: var(--champagne); width: 116%; max-width: calc(100vw - 3rem); margin-top: 0.85rem;"></div>
            </div>
            <p class="font-body"
               style="color: var(--cloud); font-size: 1rem; line-height: 1.75; margin-bottom: 2rem;">
                Call us now or fill out our short online form. We will confirm your pickup details and lock in your flat rate within the hour.
            </p>
            <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap;">
                <a href="tel:+17083154445"
                   class="font-head"
                   style="background: var(--champagne); color: var(--navy); padding: 0.95rem 2rem; font-weight: 700; font-size: 1rem; text-decoration: none; display: inline-block;">
                    <i class="fa fa-phone" style="margin-right: 0.5rem;"></i>Call (708) 315-4445
                </a>
                <button onclick="window.dispatchEvent(new Event('open-modal-quote'))"
                        class="font-head"
                        style="background: transparent; color: var(--champagne); border: 2px solid var(--champagne);
                               padding: 0.95rem 2rem; font-weight: 700; font-size: 1rem; cursor: pointer;">
                    Reserve Tailgate Bus Online
                </button>
            </div>
        </div>
    </section>

    {{-- ─── High-Margin Service Modules ───────────────────────────────────────── --}}
    <section id="lp-services"
             style="background: var(--white); padding: 4.5rem 1.5rem; scroll-margin-top: 80px;">
        <div style="max-width: 1100px; margin: 0 auto;">
            <div style="width: fit-content; margin-bottom: 1.5rem;">
                <h2 class="font-head"
                    style="font-size: clamp(1.75rem, 4vw, 2.75rem); font-weight: 400; color: var(--navy); line-height: 1.25;">
                    Popular
                    <strong style="font-weight: 700; color: var(--champagne);">Service Packages</strong>
                </h2>
                <div style="height: 3px; background: var(--champagne); width: 116%; max-width: calc(100vw - 3rem); margin-top: 0.85rem;"></div>
            </div>
            <p class="font-body"
               style="color: var(--slate); font-size: 1rem; line-height: 1.75; margin-bottom: 2.75rem; max-width: 700px;">
                Game day transportation is just the start. Stop &amp; Go Airport Shuttle Service, Inc. serves every occasion that calls for a premium vehicle and a professional chauffeur on the South Side and across Chicagoland.
            </p>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(290px, 1fr)); gap: 1.5rem;">

                <div style="border: 1px solid var(--cloud); border-top: 4px solid var(--champagne); padding: 2rem 1.5rem; display: flex; flex-direction: column;">
                    <h3 class="font-head"
                        style="color: var(--navy); font-size: 1.1rem; font-weight: 700; margin-bottom: 0.75rem; line-height: 1.4;">
                        South Side Quincea&ntilde;era &amp; Milestone Celebrations
                    </h3>
                    <p class="font-body"
                       style="color: var(--slate); font-size: 0.95rem; line-height: 1.7; flex: 1; margin-bottom: 1.25rem;">
                        We make milestone celebrations unforgettable for families across the South Side and all of Chicagoland. Stretch limousines and party buses for Quincea&ntilde;eras, Sweet 16s, anniversaries, and family milestones. Call us to plan your custom photo route and venue transfer.
                    </p>
                    <button onclick="window.dispatchEvent(new Event('open-modal-quote'))"
                            class="font-head"
                            style="background: var(--navy); color: var(--champagne); border: none; padding: 0.65rem 1.25rem; font-size: 0.875rem; font-weight: 700; cursor: pointer; align-self: flex-start;">
                        Get a Quote
                    </button>
                </div>

                <div style="border: 1px solid var(--cloud); border-top: 4px solid var(--champagne); padding: 2rem 1.5rem; display: flex; flex-direction: column;">
                    <h3 class="font-head"
                        style="color: var(--navy); font-size: 1.1rem; font-weight: 700; margin-bottom: 0.75rem; line-height: 1.4;">
                        Corporate Suite &amp; Executive Airport Transfers
                    </h3>
                    <p class="font-body"
                       style="color: var(--slate); font-size: 0.95rem; line-height: 1.7; flex: 1; margin-bottom: 1.25rem;">
                        We connect corporate guests flying into O'Hare or Midway directly to Rate Field, downtown hotels, or your office. Real-time flight tracking, white-glove luggage service, and vehicles stocked to your specifications before departure.
                    </p>
                    <button onclick="window.dispatchEvent(new Event('open-modal-quote'))"
                            class="font-head"
                            style="background: var(--navy); color: var(--champagne); border: none; padding: 0.65rem 1.25rem; font-size: 0.875rem; font-weight: 700; cursor: pointer; align-self: flex-start;">
                        Get a Quote
                    </button>
                </div>

                <div style="border: 1px solid var(--cloud); border-top: 4px solid var(--champagne); padding: 2rem 1.5rem; display: flex; flex-direction: column;">
                    <h3 class="font-head"
                        style="color: var(--navy); font-size: 1.1rem; font-weight: 700; margin-bottom: 0.75rem; line-height: 1.4;">
                        Post-Game Fine Dining &amp; Nightlife Charters
                    </h3>
                    <p class="font-body"
                       style="color: var(--slate); font-size: 0.95rem; line-height: 1.7; flex: 1; margin-bottom: 1.25rem;">
                        After the final out, let us take the group to dinner in Bridgeport, Pilsen, or downtown without anyone scrambling for a ride. We know the South Side dining corridors and build the post-game itinerary around your reservation time.
                    </p>
                    <button onclick="window.dispatchEvent(new Event('open-modal-quote'))"
                            class="font-head"
                            style="background: var(--navy); color: var(--champagne); border: none; padding: 0.65rem 1.25rem; font-size: 0.875rem; font-weight: 700; cursor: pointer; align-self: flex-start;">
                        Get a Quote
                    </button>
                </div>

            </div>
        </div>
    </section>

    {{-- ─── Reviews ────────────────────────────────────────────────────────────── --}}
    <x-sections.review-slider />

    {{-- ─── FAQ ────────────────────────────────────────────────────────────────── --}}
    <x-sections.faq preset="chicago-white-sox" />

    {{-- ─── Bottom CTA ─────────────────────────────────────────────────────────── --}}
    <section id="lp-cta-bottom"
             style="background: var(--champagne); padding: 3rem 1.5rem; text-align: center; scroll-margin-top: 80px;">
        <div style="max-width: 700px; margin: 0 auto;">
            <h2 class="font-head"
                style="font-size: clamp(1.5rem, 3.5vw, 2.25rem); font-weight: 700; color: var(--navy); margin-bottom: 0.75rem; line-height: 1.3;">
                Your Rate Field Group Ride Is One Call Away
            </h2>
            <p class="font-body"
               style="color: var(--navy); font-size: 1rem; line-height: 1.7; margin-bottom: 1.75rem; opacity: 0.85;">
                Stop &amp; Go Airport Shuttle Service, Inc. serves groups across all of Chicagoland. Call us or request a free quote online and we will match your group to the right vehicle.
            </p>
            <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap;">
                <a href="tel:+17083154445"
                   class="font-head"
                   style="background: var(--navy); color: var(--champagne); padding: 0.95rem 2.25rem; font-weight: 700; font-size: 1.05rem; text-decoration: none; display: inline-block;">
                    <i class="fa fa-phone" style="margin-right: 0.5rem;"></i>Call (708) 315-4445
                </a>
                <button onclick="window.dispatchEvent(new Event('open-modal-quote'))"
                        class="font-head"
                        style="background: transparent; color: var(--navy); border: 2px solid var(--navy);
                               padding: 0.95rem 2.25rem; font-weight: 700; font-size: 1.05rem; cursor: pointer;">
                    Reserve Tailgate Bus Online
                </button>
            </div>
        </div>
    </section>

</x-layouts.lp>
