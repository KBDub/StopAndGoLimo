@php echo '<?xml version="1.0" encoding="UTF-8"?>'; @endphp
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">

    {{-- ═══════════════════════════════════════════════════════════════════════
         CORE PAGES
    ══════════════════════════════════════════════════════════════════════════ --}}

    <url>
        <loc>https://www.top5pct.com/</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>weekly</changefreq>
        <priority>1.0</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/about</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>yearly</changefreq>
        <priority>0.7</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/company</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>yearly</changefreq>
        <priority>0.7</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/portfolio</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.7</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/contact</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>yearly</changefreq>
        <priority>0.7</priority>
    </url>

    <url>
        <loc>https://www.top5pct.com/resources</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.5</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/top5pct-merchandise</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.6</priority>
    </url>

    {{-- ═══════════════════════════════════════════════════════════════════════
         CUSTOM APPAREL — Silo + Sub-pages
    ══════════════════════════════════════════════════════════════════════════ --}}

    <url>
        <loc>https://www.top5pct.com/custom-apparel</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.9</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/custom-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/vinyl-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/printing-options/rhinestone-apparel</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/specialty-materials/glitter-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/specialty-materials/foil-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/specialty-materials/glow-in-the-dark-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/specialty-materials/flock-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/specialty-materials/reflective-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/specialty-materials/holographic-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/specialty-materials/brick-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/pattern-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/printing-options/embroidery</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/picture-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/printing-options/dtf-printing</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/group-shirts/reunion-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/group-shirts/corporate-wear-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/group-shirts/spirit-wear-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/printing-options/dye-sublimation-printing</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/printing-options/screen-printing</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/custom-apparel/specialty-materials/puff-shirts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>

    {{-- ═══════════════════════════════════════════════════════════════════════
         SIGNS — Silo + Sub-pages
    ══════════════════════════════════════════════════════════════════════════ --}}

    <url>
        <loc>https://www.top5pct.com/signs</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.9</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/business-signs/banners</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/business-signs/posters</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/table-signs/table-runners</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/ground-signs/sidewalk-signs</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/ground-signs/yard-signs</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/business-signs/window-signs</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/business-signs/wall-signs</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/business-signs/floor-signs</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/business-signs/door-signs</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/ground-signs/sidewalk-signs-a-frame-signs</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/table-signs/table-cloths</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/signs/coronavirus-signs</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>yearly</changefreq>
        <priority>0.6</priority>
    </url>

    {{-- ═══════════════════════════════════════════════════════════════════════
         VEHICLE GRAPHICS — Silo + Sub-pages
    ══════════════════════════════════════════════════════════════════════════ --}}

    <url>
        <loc>https://www.top5pct.com/vehicle-graphics</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.9</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/vehicle-graphics/automobile-graphics</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/vehicle-graphics/vehicle-magnets</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/vehicle-graphics/dot-decals</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>

    {{-- ═══════════════════════════════════════════════════════════════════════
         STICKERS — Silo + Sub-pages
    ══════════════════════════════════════════════════════════════════════════ --}}

    <url>
        <loc>https://www.top5pct.com/stickers</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.9</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/stickers/standard-stickers-decals</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/stickers/custom-shaped-stickers-decals</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>

    {{-- ═══════════════════════════════════════════════════════════════════════
         DESIGN SERVICES — Silo + Sub-pages
    ══════════════════════════════════════════════════════════════════════════ --}}

    <url>
        <loc>https://www.top5pct.com/design-services</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.9</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/design-services/logo-design</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/design-services/graphic-design</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/design-services/custom-storefronts</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>

    {{-- ═══════════════════════════════════════════════════════════════════════
         PROMOTIONAL ITEMS — Silo + Sub-pages
    ══════════════════════════════════════════════════════════════════════════ --}}

    <url>
        <loc>https://www.top5pct.com/promotional-items</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.9</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/promotional-items/mugs</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/promotional-items/can-koozies</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/promotional-items/towels</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/promotional-items/drink-coasters</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/promotional-items/tote-bags</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>
    <url>
        <loc>https://www.top5pct.com/promotional-items/mouse-pads</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.8</priority>
    </url>

    {{-- ═══════════════════════════════════════════════════════════════════════
         SERVICE AREAS — Index + All 41 city pages (dynamic from PrimaryLocations)
    ══════════════════════════════════════════════════════════════════════════ --}}

    <url>
        <loc>https://www.top5pct.com/service-areas</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.7</priority>
    </url>
    @foreach(\App\Data\PrimaryLocations::all() as $loc)
    @php
        $cityName = $loc['city'] ?? $loc['name'];
        $citySlug = \Illuminate\Support\Str::slug($cityName . '-' . $loc['state']);
        $isHq     = $cityName === 'Joliet';
    @endphp
    <url>
        <loc>https://www.top5pct.com/service-areas/{{ $citySlug }}</loc>
        <lastmod>2026-05-01</lastmod>
        <changefreq>monthly</changefreq>
        <priority>{{ $isHq ? '0.8' : '0.6' }}</priority>
    </url>
    @endforeach

    {{-- ═══════════════════════════════════════════════════════════════════════
         E-COMMERCE — Published products (dynamic from Lunar DB)
    ══════════════════════════════════════════════════════════════════════════ --}}

    @foreach($products as $product)
    <url>
        <loc>https://www.top5pct.com/products/{{ $product->slug }}</loc>
        <lastmod>{{ $product->updated_at->format('Y-m-d') }}</lastmod>
        <changefreq>weekly</changefreq>
        <priority>0.8</priority>
    </url>
    @endforeach

    {{-- ═══════════════════════════════════════════════════════════════════════
         E-COMMERCE — Collections (dynamic from Lunar DB)
    ══════════════════════════════════════════════════════════════════════════ --}}

    @foreach($collections as $collection)
    <url>
        <loc>https://www.top5pct.com/collections/{{ $collection->slug }}</loc>
        <lastmod>{{ $collection->updated_at->format('Y-m-d') }}</lastmod>
        <changefreq>weekly</changefreq>
        <priority>0.7</priority>
    </url>
    @endforeach

</urlset>
